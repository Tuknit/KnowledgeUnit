...
public class Controller {
	...
	public String getProjectList() {
		...
		for (int i = 0; i < projects.length; i++) {
			...
			if (projects[i] != null) {
				msg += "\n" + (i) + ". " + projects[i].getIdProject() + " - " + projects[i].getProjectName();
			}
		...
		for(int i=0; i<projects.length; i++){
			...
			if(projects[i]!=null){
				...
				for(int j =0; j < 6; j++){
					...
					if(projects[i].getStageCapsuleApprove(j) != null){

						msg += "\n"+projects[i].getStageCapsuleApprove(j);
	...
	public boolean registerKnowledgeUnit(int projectPosition, String id, int capsuleInformation, String description, int type, String learnedLessons) {
		...
		projects[projectPosition].stageCapsule(newKnowledgeUnit, capsuleInformation);
	...
	public String consultRegistratedCapsules(){
		...
		for(int i=0; i<projects.length; i++){
			...
			if(projects[i]!=null){
				...
				msg += "\n"+ "Not approve capsules:";

				for(int j =0; j < 6; j++){

					if(projects[i].getStageCapsuleNotApprove(j) != null){

						msg += "\n"+projects[i].getStageCapsuleNotApprove(j);
	...
	public String capsulePublication(){
		...
		htmlBuilder.append("<title>Título de la página</title>");
		htmlBuilder.append("</head>");
		htmlBuilder.append("<body>");
		htmlBuilder.append("<h1>Encabezado de la página</h1>");
		htmlBuilder.append("<p>Este es un párrafo de texto.</p>");
		htmlBuilder.append("</body>");
		htmlBuilder.append("</html>");
